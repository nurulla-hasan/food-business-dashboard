...
const EditMenuModal = ({ isOpen, onOpenChange, menu, onSubmit, loading }) => {
  console.log('EditMenuModal props:', { isOpen, menu });
  const form = useForm({
  ...
  useEffect(() => {
    if (menu) {
      const formValues = {
        ...
        weekStart: menu.weekStart ? new Date(menu.weekStart) : undefined,
        weekEnd: menu.weekEnd ? new Date(menu.weekEnd) : undefined,
